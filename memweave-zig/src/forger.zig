//! Forger — the configuration surface the behaviour factory forges.
//!
//! `config.zig` next to this file declares the same eleven config structs by
//! hand, each restating its field list, its defaults and a `validate()`
//! transcribed from the matching Python `__post_init__`. This file declares
//! none of them. Every type below is generated at comptime from the
//! descriptors in `src/behaviors/<name>/`, where each atomic behaviour owns a
//! folder holding `manifest.yml` (identity), `schema.yml` (shape and rules)
//! and `config.yml` (this profile's values). See `src/behaviors/README.md`
//! for the descriptor format and `src/factory.zig` for the mechanism.
//!
//! The two files coexist deliberately: `config.zig` is the hand-written
//! reference, and the parity tests at the bottom of this one assert — field
//! by field, default by default — that the forged types match it. When they
//! agree, the descriptors are a faithful description of the port; when they
//! drift, the build says so.
//!
//! What this file is: the facade. It names the behaviours the library ships
//! with and re-exports their generated types under the same names the Python
//! original uses.
//!
//! Two things differ from the hand-written structs, both because a generated
//! struct type has fields but no methods:
//!
...
const reference = @import("config.zig");
...
// -- Config types ------------------------------------------------------------
//
// Same names as the hand-written structs in `config.zig`, so callers and the
// Python parity mapping read identically either way.
...
// -- Shipped defaults --------------------------------------------------------
//
// Read out of the generated profiles rather than declared here.
// `src/behaviors/flush/config.yml` is the source of truth for the flush
// prompt on this side.
...
// -- Tests -------------------------------------------------------------------
//
// The first block repeats the assertions `config.zig` carries, re-pointed at
// the generated modules: the port's parity with Python is what is being
// checked, not the factory's plumbing (that is tested in `src/factory/`).
// The block at the end then checks the forged types against the hand-written
// ones directly.
...
// -- Parity with the hand-written structs ------------------------------------
//
// The point of keeping `config.zig` alongside this file: it is an independent
// second opinion on what the port should produce. These walk both sides with
// reflection, so a descriptor that drifts from the hand-written reference
// fails the build rather than quietly shipping a different default.

/// Every field of `Forged` must exist in `Reference` with the same name, in
/// the same position, with the same type and the same default value.
///
/// A composed field is the one place the types legitimately differ: the
/// forged `QueryConfig.hybrid` is the *generated* `HybridConfig`, not the
/// hand-written one. There the check recurses into the shape instead of
/// demanding type identity.
fn expectSameShape(comptime Forged: type, comptime Reference: type) !void {
    const gen = @typeInfo(Forged).@"struct".fields;
    const hand = @typeInfo(Reference).@"struct".fields;
    try std.testing.expectEqual(hand.len, gen.len);

    inline for (gen, hand) |g, h| {
        try std.testing.expectEqualStrings(h.name, g.name);
        if (comptime @typeInfo(g.type) == .@"struct") {
            try expectSameShape(g.type, h.type);
        } else {
            try std.testing.expectEqual(h.type, g.type);
        }

        const g_default = comptime g.defaultValue();
        const h_default = comptime h.defaultValue();
        try std.testing.expect((g_default == null) == (h_default == null));
        if (g_default) |gd| try expectSameValue(gd, h_default.?);
    }
}

/// Structural equality that reaches into composed configs and compares
/// strings and string lists by content rather than by pointer.
fn expectSameValue(forged: anytype, expected: anytype) !void {
    const T = @TypeOf(forged);
    if (comptime T == []const u8) return std.testing.expectEqualStrings(expected, forged);
    if (comptime T == []const []const u8) {
        try std.testing.expectEqual(expected.len, forged.len);
        for (forged, expected) |f, e| try std.testing.expectEqualStrings(e, f);
        return;
    }
    switch (@typeInfo(T)) {
        .optional => {
            try std.testing.expect((forged == null) == (expected == null));
            if (forged) |f| try expectSameValue(f, expected.?);
        },
        // A composed behaviour: same walk, one level down, on the values.
        .@"struct" => inline for (@typeInfo(T).@"struct".fields) |sf| {
            try expectSameValue(@field(forged, sf.name), @field(expected, sf.name));
        },
        else => try std.testing.expectEqual(expected, forged),
    }
}

test "every forged config has the same shape and defaults as its hand-written twin" {
    try expectSameShape(EmbeddingConfig, reference.EmbeddingConfig);
    try expectSameShape(ChunkingConfig, reference.ChunkingConfig);
    try expectSameShape(HybridConfig, reference.HybridConfig);
    try expectSameShape(MMRConfig, reference.MMRConfig);
    try expectSameShape(TemporalDecayConfig, reference.TemporalDecayConfig);
    try expectSameShape(QueryConfig, reference.QueryConfig);
    try expectSameShape(CacheConfig, reference.CacheConfig);
    try expectSameShape(SyncConfig, reference.SyncConfig);
    try expectSameShape(FlushConfig, reference.FlushConfig);
    try expectSameShape(VectorConfig, reference.VectorConfig);
    try expectSameShape(MemoryConfig, reference.MemoryConfig);
}

test "the forged flush prompt is byte-identical to the hand-written one" {
    try std.testing.expectEqualStrings(reference.default_flush_system_prompt, default_flush_system_prompt);
}

test "forged and hand-written validate() agree on the same inputs" {
    // Sampled across every kind of rule the schemas declare: a bound, an
    // optional's "unlimited when null", a cross-field invariant, a sum, and a
    // cascade through two levels of composition.
    try std.testing.expectEqual(
        reference.EmbeddingConfig.validate(.{ .timeout = 0 }),
        Embedding.validate(.{ .timeout = 0 }),
    );
    try std.testing.expectEqual(
        reference.ChunkingConfig.validate(.{ .tokens = 10, .overlap = 10 }),
        Chunking.validate(.{ .tokens = 10, .overlap = 10 }),
    );
    try std.testing.expectEqual(
        reference.HybridConfig.validate(.{ .vector_weight = 0.5, .text_weight = 0.3 }),
        Hybrid.validate(.{ .vector_weight = 0.5, .text_weight = 0.3 }),
    );
    try std.testing.expectEqual(
        reference.CacheConfig.validate(.{ .max_entries = null }),
        Cache.validate(.{ .max_entries = null }),
    );
    try std.testing.expectEqual(
        reference.CacheConfig.validate(.{ .max_entries = 0 }),
        Cache.validate(.{ .max_entries = 0 }),
    );
    try std.testing.expectEqual(
        reference.MemoryConfig.validate(.{ .query = .{ .mmr = .{ .lambda_param = 2.0 } } }),
        Memory.validate(.{ .query = .{ .mmr = .{ .lambda_param = 2.0 } } }),
    );
    try std.testing.expectEqual(
        reference.MemoryConfig.validate(.{}),
        Memory.validate(.{}),
    );
}

test "the forged derivations match the hand-written properties" {
    const c = ChunkingConfig{ .tokens = 400, .overlap = 80 };
    const h = reference.ChunkingConfig{ .tokens = 400, .overlap = 80 };
    try std.testing.expectEqual(h.maxChars(), Chunking.ops.maxChars(c));
    try std.testing.expectEqual(h.overlapChars(), Chunking.ops.overlapChars(c));

    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();

    const m = MemoryConfig{ .workspace_dir = "/tmp/project" };
    const hm = reference.MemoryConfig{ .workspace_dir = "/tmp/project" };
    try std.testing.expectEqualStrings(
        try hm.resolvedDbPath(allocator),
        try Memory.ops.resolvedDbPath(m, allocator),
    );
    try std.testing.expectEqualStrings(
        try hm.memoryDir(allocator),
        try Memory.ops.memoryDir(m, allocator),
    );
}
